...
class RepresentativePhoneNumberTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = userAnswers =>
    transformFromDeparture(
      ...
      extractDataFromDepartureData = _.departureData.Representative.flatMap(_.ContactPerson.map(_.phoneNumber)).toSeq,
      generateCapturedAnswers = representativePhoneNumber =>
        representativePhoneNumber.map(
          repPhoneNumber => (RepresentativePhoneNumberPage, repPhoneNumber)
        )
    )
}
