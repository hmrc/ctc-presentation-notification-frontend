  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = {
    userAnswers =>
      transformFromDeparture(
        ...
        extractDataFromDepartureData = _.departureData.Consignment.PlaceOfLoading.flatMap(_.UNLocode).toSeq,
        generateCapturedAnswers = unLocodes =>
          unLocodes.map {
            unLocode => (UnLocodePage, unLocode)
          }
      )
  }
}
