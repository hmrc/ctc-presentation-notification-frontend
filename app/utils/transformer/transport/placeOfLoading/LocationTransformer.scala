...
class LocationTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = {
    userAnswers =>
      transformFromDeparture(
        ...
        extractDataFromDepartureData = _.departureData.Consignment.PlaceOfLoading.flatMap(_.location).toSeq,
        generateCapturedAnswers = locations =>
          locations.map {
