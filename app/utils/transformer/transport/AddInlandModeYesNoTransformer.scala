...
class AddInlandModeYesNoTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = userAnswers =>
    transformFromDeparture(
      ...
      generateCapturedAnswers = inlandMode => Seq((AddInlandModeOfTransportYesNoPage, inlandMode.nonEmpty))
    )
}
