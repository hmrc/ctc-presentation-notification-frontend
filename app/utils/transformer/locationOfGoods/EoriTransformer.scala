...
class EoriTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = userAnswers =>
    transformFromDeparture(
      ...
      extractDataFromDepartureData = _.departureData.Consignment.LocationOfGoods.flatMap(_.EconomicOperator.map(_.identificationNumber)).toSeq,
      generateCapturedAnswers = _.map((EoriPage, _))
    )
}
