...
class NameTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = userAnswers =>
    transformFromDeparture(
      ...
      extractDataFromDepartureData = _.departureData.Consignment.LocationOfGoods.flatMap(_.ContactPerson.map(_.name)).toSeq,
      generateCapturedAnswers = _.map((NamePage, _))
    )
}
