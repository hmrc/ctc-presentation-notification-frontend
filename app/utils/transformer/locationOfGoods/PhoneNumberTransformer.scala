...
class PhoneNumberTransformer extends PageTransformer {
  ...
  override def transform(implicit hc: HeaderCarrier): UserAnswers => Future[UserAnswers] = userAnswers =>
    transformFromDeparture(
      ...
      extractDataFromDepartureData = _.departureData.Consignment.LocationOfGoods.flatMap(_.ContactPerson.map(_.phoneNumber)).toSeq,
      generateCapturedAnswers = _.map((PhoneNumberPage, _))
    )
}
