  ...
  "LocationOfGoodsIdentification Controller" - {
    ...
    "when value is inferred" - {
      "must redirect to next page" in {
        ...
        userAnswersCaptor.getValue.getValue(InferredIdentificationPage) mustEqual id1
      ...
      "must return OK and the correct view for a GET" in {
        ...
        contentAsString(result) mustEqual
          view(form, departureId, ids, mode)(request, messages).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      contentAsString(result) mustEqual
        view(boundForm, departureId, ids, mode)(request, messages).toString
    }
