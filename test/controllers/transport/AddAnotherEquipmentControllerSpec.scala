...
class AddAnotherEquipmentControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with Generators {
  ...
  private lazy val addAnotherEquipmentRoute = transportRoutes.AddAnotherEquipmentController.onPageLoad(departureId, mode).url
    ...
    "when 0 equipment" - {
      "must redirect to add equipment yes/no page" in {
        ...
        redirectLocation(result).value mustEqual
          transportRoutes.AddTransportEquipmentYesNoController.onPageLoad(departureId, mode).url
      }
    }
