...
class RemoveSealYesNoControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private val form                      = formProvider("transport.equipment.index.seals.transport.equipment.removeSeal", sealIdNumber)
  private val mode                      = NormalMode
  private lazy val removeSealYesNoRoute = sealRoutes.RemoveSealYesNoController.onPageLoad(departureId, mode, equipmentIndex, sealIndex).url
  ...
  "RemoveSealYesNo Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      contentAsString(result) mustEqual
        view(form, departureId, mode, equipmentIndex, sealIndex, sealIdNumber)(request, messages).toString
    }
    ...
    "must redirect to the next page" - {
      "when yes is submitted" in {
        ...
        redirectLocation(result).value mustEqual controllers.transport.equipment.index.routes.AddAnotherSealController
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      contentAsString(result) mustEqual
        view(boundForm, departureId, mode, equipmentIndex, sealIndex, sealIdNumber)(request, messages).toString
    }
