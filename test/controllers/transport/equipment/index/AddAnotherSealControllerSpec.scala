...
import controllers.transport.equipment.index.routes as equipmentRoutes
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to seal id number page at next index" in {
          ...
          userAnswersCaptor.getValue.getValue(AddAnotherSealPage(equipmentIndex)) mustEqual true
        }
      }
