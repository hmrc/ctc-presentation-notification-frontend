...
import controllers.transport.equipment.routes as equipmentRoutes
...
class ApplyAnotherItemControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar with Generators {
  ...
  private lazy val applyAnotherItemRoute = equipmentRoutes.ApplyAnotherItemController.onPageLoad(departureId, mode, equipmentIndex).url
  ...
  "ApplyAnotherItem Controller" - {
    ...
    "redirect to select items page" - {
      "when 0 items" in {
        ...
        redirectLocation(result).value mustEqual
          equipmentRoutes.SelectItemsController.onPageLoad(departureId, mode, equipmentIndex, itemIndex).url
      }
    }
    ...
    "must return OK and the correct view for a GET" - {
      "when max limit not reached" in {
        ...
        contentAsString(result) mustEqual
          view(form(notMaxedOutViewModel, equipmentIndex), notMaxedOutViewModel)(request, messages, frontendAppConfig).toString
      }
      ...
      "when max limit reached" in {
        ...
        contentAsString(result) mustEqual
          view(form(maxedOutViewModel, equipmentIndex), maxedOutViewModel)(request, messages, frontendAppConfig).toString
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered" - {
      "when max limit not reached" in {
        ...
        setExistingUserAnswers(emptyUserAnswers.setValue(ApplyAnotherItemPage(equipmentIndex), true))
    ...
    "must redirect to next page" - {
      ...
      "when max limit not reached" - {
        "when yes submitted" in {
          ...
          userAnswersCaptor.getValue.getValue(ApplyAnotherItemPage(equipmentIndex)) mustEqual true
        }

        "when no submitted" in {
          when(mockViewModelProvider.apply(any(), any(), any(), any(), any())(any()))
            .thenReturn(notMaxedOutViewModel)
          ...
          val userAnswersCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswersCaptor.capture())
          userAnswersCaptor.getValue.getValue(ApplyAnotherItemPage(equipmentIndex)) mustEqual false
        }
      }
