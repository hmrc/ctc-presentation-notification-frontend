...
class LocationOfGoodsPageSpec extends PageBehaviours {

  "LocationOfGoodsPage" - {

    beRetrievable[LocationOfGoodsIdentification](LocationOfGoodsPage)

    beSettable[LocationOfGoodsIdentification](LocationOfGoodsPage)

    beRemovable[LocationOfGoodsIdentification](LocationOfGoodsPage)
      ...
      "must clean up LocationOfGoodsIdentifierSection and inferred value" in {
        forAll(arbitrary[LocationOfGoodsIdentification]) {
          qualifierOfIdentification =>
            ...
            val result = userAnswers.setValue(LocationOfGoodsPage, qualifierOfIdentification)

            result.get(LocationOfGoodsPage) mustBe defined
