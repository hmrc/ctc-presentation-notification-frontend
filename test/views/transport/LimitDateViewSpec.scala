...
class LimitDateViewSpec extends DateInputViewBehaviours {
  ...
  override def applyView(form: Form[LocalDate]): HtmlFormat.Appendable =
    injector.instanceOf[LimitDateView].apply(form, NormalMode, departureId, maxDate, officeOfDestination)(fakeRequest, messages)

  override val prefix: String = "transport.limit.date"
