      ...
      "must return 1 section" in {
        forAll(arbitrary[Identification], arbitrary[Mode]) {
          (identification, mode) =>
            ...
            val sections = new ActiveBorderAnswersViewModelProvider().apply(userAnswers, departureId, mode, activeIndex).sections
            ...
            "must return 6 rows and addAnotherLink is not defined" in {
              forAll(arbitrarySecurityDetailsNonZeroType.arbitrary,
                ...
                (securityType, identification, identificationNumber, office, nationality, conveyanceRefNumber) =>
                  ...
                  val sections            = viewModelProvider.apply(answers, departureId, mode, activeIndex).sections
        ...
        "add another link should be defined in section 2 and not 1" in {
          forAll(
            ...
            (identification1, identification2) =>
              ...
              val viewModelProvider         = injector.instanceOf[ActiveBorderAnswersViewModelProvider]
              val activeBorderMeansSection1 = viewModelProvider.apply(answers, departureId, mode, Index(0)).sections
              val activeBorderMeansSection2 = viewModelProvider.apply(answers, departureId, mode, Index(1)).sections
              val activeBorderSection1      = activeBorderMeansSection1.head
              val activeBorderSection2      = activeBorderMeansSection2.head
    ...
    "must return 1 section with a add another link when ie15 customs office of transit is defined" in {
      forAll(arbitrary[Mode]) {
        mode =>
          ...
          val sections = new ActiveBorderAnswersViewModelProvider().apply(ie015WithActiveBorderTransportAnswer, departureId, mode, activeIndex).sections
          sections.size mustBe 1
